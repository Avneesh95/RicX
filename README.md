...
- Product reviews & ratings (only for products you've actually received — enforced server-side against delivery status)
- Wishlist and cart
- Dark mode
...
- Coupon codes at checkout — validated and priced server-side, discount reflected in the actual payment amount
- Order confirmation email sent automatically (COD: immediately; Razorpay: after payment verification)
...
- In-app toast notifications for all actions (no browser `alert()` popups)
